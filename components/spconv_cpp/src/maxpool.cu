...
#pragma unroll
        for (int ilp = 0; ilp < NumILP; ++ilp) {
            ...
            reinterpret_cast<VecType *>(bufo)[0] = reinterpret_cast<VecType *>(outFeatures)[idxo];
            reinterpret_cast<VecType *>(bufi)[0] = reinterpret_cast<const VecType *>(inFeatures)[idxi];
#pragma unroll
            ...
            reinterpret_cast<VecType *>(outFeatures)[idxo] = reinterpret_cast<VecType *>(bufo)[0];
        }
    }
}
...
#pragma unroll
        for (int ilp = 0; ilp < NumILP; ++ilp) {
            idxi = indicesIn[ix + ILPStrideY[ilp]] * numPlanes + threadIdx.x;
            idxo = indicesOut[ix + ILPStrideY[ilp]] * numPlanes + threadIdx.x;
            reinterpret_cast<VecType *>(bufo)[0] = reinterpret_cast<const VecType *>(outFeatures)[idxo];
            reinterpret_cast<VecType *>(bufi)[0] = reinterpret_cast<const VecType *>(inFeatures)[idxi];
            reinterpret_cast<VecType *>(bufdo)[0] = reinterpret_cast<const VecType *>(dout)[idxo];
...
void maxpool_fwd_cuda(torch::Tensor outFeatures, torch::Tensor inFeatures,
    ...
    tv::DispatchTorch<float_types_t>()(dtype, [&](auto TValue) {
        ...
        tv::DispatchTorch<int_types_t>()(int_dtype, [&](auto IndexValue) {
            ...
            tv::mp_for_each<kernel_block_t>([=, &outFeatures, &inFeatures, &indicesIn, &indicesOut, &notFound](auto NumTLP) {
                ...
                if (notFound) {
                    if (numPlanes % NumTLP == 0) {
                        if (numHotBlock >= NumTLP) {
                            ...
                            <<<dim3(std::min(size / NumTLP, 512), numPlanes / NumTLP), dim3(NumTLP / vecloadFactor, NumTLP / NumILP), 0, stream>>>(
                        ...
                        if (size > numHotBlock) {
                            ...
                            <<<dim3(1, numPlanes / NumTLP), dim3(NumTLP / NumILP, NumTLP),0, stream>>>(
            ...
            if (notFound) {
                ...
                if (numHotBlock >= NumTLP) {
                    ...
                    <<<dim3(size / NumTLP, tv::cuda::DivUp(numPlanes, NumTLP)),dim3(NumTLP / NumILP, NumTLP), 0, stream>>>(
                            outFeatures.data_ptr<T>(), inFeatures.data_ptr<T>(),
                            indicesIn.data_ptr<Index>(), indicesOut.data_ptr<Index>(),
                ...
                if (size > numHotBlock) {
                    ...
                    <<<dim3(1, tv::cuda::DivUp(numPlanes, NumTLP)),dim3(NumTLP / NumILP, NumTLP), 0, stream>>>(
                            outFeatures.data_ptr<T>(), inFeatures.data_ptr<T>(),
                            indicesIn.data_ptr<Index>() + numHotBlock,
                            indicesOut.data_ptr<Index>() + numHotBlock,
                            size - numHotBlock, numPlanes);
...
void maxpool_bwd_cuda(torch::Tensor outFeatures, torch::Tensor inFeatures,
    ...
    tv::DispatchTorch<float_types_t>()(dtype, [&](auto TValue) {
        ...
        tv::DispatchTorch<int_types_t>()(int_dtype, [&](auto IndexValue) {
            ...
            bool notFound = true;
            constexpr int vecloadFactor = sizeof(vecload_type_t) / sizeof(T);
            tv::mp_for_each<kernel_block_t>([=, &outFeatures, &inFeatures, &dout, &din, &indicesIn, &indicesOut, &notFound](auto NumTLP) {
                constexpr int NumILP = NumTLP / 4;
                int numHotBlock = (size / NumTLP) * NumTLP;
                if (notFound) {
                    if (numPlanes % NumTLP == 0) {
