...
import spconv_lite
import spconv_lite.functional as Fsp
from spconv_lite import ops
from spconv_lite.modules import SparseModule
...
class SparseConvolution(SparseModule):
    ...
    def forward(self, input):
        assert isinstance(input, spconv_lite.SparseConvTensor)
        ...
        if self.conv1x1:
            ...
            if self.bias is not None:
                features += self.bias
            out_tensor = spconv_lite.SparseConvTensor(features,
                                                      input.indices,
                                                      input.spatial_shape,
                                                      input.batch_size)
        ...
        out_tensor = spconv_lite.SparseConvTensor(out_features, outids, out_spatial_shape, batch_size)
        out_tensor.indice_dict = input.indice_dict
        out_tensor.grid = input.grid
        return out_tensor
...
class SubMConv2d(SparseConvolution):
    ...
    SSC picks active site only when point on the center of kernel.
    Sparse convolution (SC) picks active site when point inside kernel.
    in code, only strategy of generating sparse index is different
    more read "Submanifold Sparse Convolutional Networks"
