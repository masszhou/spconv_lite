...
def generate_sparse_data(shape: List[int],
                         num_points: List[int],
                         num_channels: List[int],
                         integer: bool = False,
                         data_range: Tuple = (-1, 1),
                         with_dense: bool = True,
                         dtype=np.float32) -> Dict[str, np.ndarray]:
    """
    generate dummy sparse matrix

    :param shape: e.g. [19, 18, 17]
    :param num_points: e.g. [1000]
    :param num_channels: e.g. [64]
    :param integer: False, use float data
    :param data_range:
    :param with_dense: if output reference dense matrix for comparison
    :param dtype: numpy dtype
    :return: a dict contains
        "features": np.ndarray (1000, 64)
        "indices": np.ndarray (1000, 4), 4 -> x,y,z,batch
        "features_dense"
    """
    dense_shape = shape
...
@contextmanager
def timeit_context(description: str, run_count: float = 1.0):
    starttime = timeit.default_timer()
    yield
    elapsedTime = timeit.default_timer() - starttime
    print(f"{description} time is :{elapsedTime/run_count:.6f}")
